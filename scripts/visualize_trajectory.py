"""Export OT/SB soft-trajectory diagnostic maps and graph summaries."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", required=True, help="Path to a visualization YAML/JSON/TOML config.")
    parser.add_argument("--dry-run", action="store_true", help="Validate and summarize without running PyTorch.")
    parser.add_argument("--output-dir", help="Override visualization.output_dir.")
    parser.add_argument(
        "--set",
        action="append",
        default=[],
...
def main() -> None:
    ...
    if args.output_dir:
        config.setdefault("visualization", {})["output_dir"] = args.output_dir
    runner = VisualizationRunner(config)
    if args.dry_run or config.get("runtime", {}).get("dry_run", False):
        runner.dry_run()
    else:
        runner.run(kind="trajectory")
